...
const ChatSessionDrawer: React.FC<ChatSessionDrawerProps> = (props) => {
  ...
  const handleDelete = useCallback(
    async (sessionId: string) => {
      const session = sessions.find((s: IAgentScopeRuntimeWebUISession) => s.id === sessionId) as
        | ExtendedChatSession
        | undefined;
      ...
      if (currentSessionId === sessionId) {
        const next = sessions.filter((s: IAgentScopeRuntimeWebUISession) => s.id !== sessionId);
  ...
  const handleEditSubmit = useCallback(async () => {
    ...
    const session = sessions.find((s: IAgentScopeRuntimeWebUISession) => s.id === editingSessionId) as
      | ExtendedChatSession
      | undefined;
    const backendId = session ? getBackendId(session) : null;
