...
def _compute_l2_scorecard(state: dict[str, Any]) -> dict[str, Any]:
...
def _grade_l2_assessment(
    ...
    *,
    ...
    risk_labels = _derive_risk_labels(scorecard, probes)
...
def _normalize_grade_thresholds(
    ...
    if not (a > b > c):
        raise click.ClickException("Invalid grade thresholds: require A > B > C and each in [0, 1].")
    return a, b, c


def _sort_assessment_items(
    ...
    *,
    ...
    # default: risk desc (high-risk first)
    return sorted(
        items,
...
def _summarize_risk_label_hits(items: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for item in items:
        if not isinstance(item, dict):
            continue
...
def assess_l2_cmd(
    ...
    threshold_a, threshold_b, threshold_c = _normalize_grade_thresholds(
        grade_a,
        grade_b,
        grade_c,
    )
    ...
    if all_projects:
        ...
        for item_project_id, path in discovered:
            ...
            scorecard = _compute_l2_scorecard(state) if state_error is None else None
            probes = (
        ...
        items = _sort_assessment_items(items, sort_by=sort_by)
        if top > 0:
            items = items[:top]

        risk_label_hits = _summarize_risk_label_hits(items)
    ...
    if not probe_only:
        ...
        scorecard = _compute_l2_scorecard(state)
