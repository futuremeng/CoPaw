    ...
    if mode in full_required_modes:
        try:
            ...
            return {
                ...
                "reason": (
                    "HanLP full dependencies are missing. Install with: "
                    "pip install 'hanlp[full]' in a dedicated Python env."
                ),
                "python_version": version_text(),
                "tokens": [],
            }
...
class HanLPSidecarRuntime:
    ...
    def ner_status(self, config: KnowledgeConfig | None) -> dict[str, str]:
        return self.task_status("ner_msra", config)

    def dep_status(self, config: KnowledgeConfig | None) -> dict[str, str]:
        return self.task_status("dep", config)

    def run_ner(self, text: str, config: KnowledgeConfig | None) -> tuple[Any, dict[str, str]]:
        return self.run_task("ner_msra", text, config)

    def run_dep(self, text: str, config: KnowledgeConfig | None) -> tuple[Any, dict[str, str]]:
        return self.run_task("dep", text, config)


class _PlaceholderRuntime:
    """Generic NLP placeholder runtime before RexUniNLU integration."""
    ...
    def probe(self, config: KnowledgeConfig | None) -> dict[str, str]:
        return self._state(
            ...
            reason=(
                "NLP provider placeholder is active. "
    ...
    def tokenize(self, text: str, config: KnowledgeConfig | None) -> tuple[list[str], dict[str, str]]:
        ...
        return [], self._state(
            ...
            reason="Semantic tokenization is unavailable before provider integration.",
        )

    def task_status(self, task_key: str, config: KnowledgeConfig | None) -> dict[str, str]:
        _ = task_key
        return self._state(
            config,
            status="unavailable",
    ...
    def run_task(self, task_key: str, text: str, config: KnowledgeConfig | None) -> tuple[Any, dict[str, str]]:
        _ = (task_key, text)
        return None, self._state(
            config,
            status="unavailable",
            reason_code="NLP_ENGINE_TASK_NOT_IMPLEMENTED",
            reason="Task execution is unavailable before provider integration.",
        )
...
class NLPRuntime:
    ...
    def __init__(self) -> None:
        self._hanlp = HanLPSidecarRuntime()
        self._placeholder = _PlaceholderRuntime()
    ...
    def _runtime(self, config: KnowledgeConfig | None) -> Any:
        if self._provider(config) == "hanlp":
            return self._hanlp
        return self._placeholder

    def probe(self, config: KnowledgeConfig | None) -> dict[str, str]:
        return self._runtime(config).probe(config)
    ...
    def task_status(self, task_key: str, config: KnowledgeConfig | None) -> dict[str, str]:
        return self._runtime(config).task_status(task_key, config)

    def run_task(self, task_key: str, text: str, config: KnowledgeConfig | None) -> tuple[Any, dict[str, str]]:
        return self._runtime(config).run_task(task_key, text, config)

    def ner_status(self, config: KnowledgeConfig | None) -> dict[str, str]:
        return self._runtime(config).task_status("ner_msra", config)

    def dep_status(self, config: KnowledgeConfig | None) -> dict[str, str]:
        return self._runtime(config).task_status("dep", config)

    def run_ner(self, text: str, config: KnowledgeConfig | None) -> tuple[Any, dict[str, str]]:
        return self._runtime(config).run_task("ner_msra", text, config)

    def run_dep(self, text: str, config: KnowledgeConfig | None) -> tuple[Any, dict[str, str]]:
        return self._runtime(config).run_task("dep", text, config)
