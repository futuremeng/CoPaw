# -*- coding: utf-8 -*-
"""Compatibility wrapper for memify enrichment jobs."""
...
async def memify_run(
	...
	if pipeline not in _PIPELINE_WHITELIST:
		return ToolResponse(content=[TextBlock(type="text", text="Error: pipeline_type is invalid or not allowed.")])
	...
	try:
		...
		return ToolResponse(content=[TextBlock(type="text", text=f"Error: memify run failed due to\n{exc}")])